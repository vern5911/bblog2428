...
#        : Backed out the Heroku implementation due to incompatibility and fees
#        : 06-Jul-25, added get_nextid() to obtain latest editable record as default
#        :   for index.html.
#       
import os
import sqlite3
...
# Compute next_id - next unedited Table record
def get_nextid():
    # Query DB table for latest unedited record Id (Bid)
    conn = get_db_connection()
    cur = conn.cursor()
    sql_ = """SELECT MIN(Bid) FROM bblog2428 
                    WHERE Feedback IS NULL 
                    ORDER BY Bid ASC 
                    LIMIT 1;"""
    next_bid = cur.execute(sql_).fetchone()
    for nid in next_bid:
        nb = nid 
    return nb
...
ic('Today Id: ',Id)
ic('Next unedited Id: ', get_nextid())

def form_dt(Bdt):
    # Create components of the current date
    yr=int(Bdt[0:4]); mo=int(Bdt[5:7]); dy=int(Bdt[8:10])
    return date(yr,mo,dy).strftime("%a %Y-%m-%d")

# Obtain the readings for a specific table id value
def get_readings(r_id):
    con = get_db_connection()
    nxt_id = get_nextid()
    reads = con.execute("""SELECT Bid, Bdate, Book, Chapter, Feedback
                            FROM bblog2428
...
# Generate a report based on the localhost:5000/yym/report browser input
...
def report(yrm):
    ...
    conn = get_db_connection()
    sql_ = """SELECT * FROM bblog2428 
                        WHERE substr(Bdate,1,4)=? AND substr(Bdate,6,2)=?
                        ORDER BY Bid ASC """
